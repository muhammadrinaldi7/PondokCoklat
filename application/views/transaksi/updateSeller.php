...
<!-- DataTales Example -->
    <div class="card shadow mb-4" style="width:50%">
        <div class="card-body">
            <?php ?>
            <form method="POST" action="<?= base_url('transaksi/Seller/updateDataAksi'); ?>">
            <?php foreach ($penjualan as $p): ?>
            <?php endforeach; ?>
                <div class="form-group">
                    <input type="hidden" name="id" value="<?= $p->id ?>" id="">
                    <label for="">Kode Outlet</label>
                    <input type="text" name="kode_outlet" readonly value="<?= $p->kode_outlet ?>" class="form-control col-md-5">
                    <?= form_error('kode_supplier','<div class="text-small text-danger"></div>') ?>
                </div>
                <div class="form-group">
                    <label for="">Nama Bahan</label>
                    <select name="kode_bahan"class="form-control">
                        <option value="<?= $p->kode_bahan ?>"><?= $p->nama_bahan ?></option>
                ...
                <div class="form-group">
                    ...
                    <input required type="number" value=<?= $p->qty ?> name="qty" onkeyup="hitung();" id="qty" class="form-control col-md-5">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="tanggal" value=<?= $p->tanggal ?> class="form-control col-md-5">
                </div>
