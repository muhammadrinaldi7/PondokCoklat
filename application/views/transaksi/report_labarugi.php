...
<body onLoad="window.print()">
    ...
    <table class="table table-striped table-bordered" width="80%">
        <thead>
            <tr>
                        <th>Pembelian</th>    
                        <?php foreach($liat as $pem) : ?>
                            <?php endforeach; ?>
                        <th><?= "Rp. ".number_format($pem->TotalPengeluaran,2,',','.'); ?></th>
                        
            </tr>
            <tr>
            <th>Penjualan</th>    
                        <?php foreach($liat1 as $pem1) : ?>
                        <?php endforeach; ?>
                        <th><?= "Rp. ".number_format($pem1->TotalPendapatan,2,',','.');
                         ?></th>
            </tr>
            <tr>
                <th>
                    <?php 
                    if(($pem1->TotalPendapatan - $pem->TotalPengeluaran) > 0){ ?>
                    Dengan ini kita mendapat Laba Keuntungan Sebesar :
                    <th>
                     <?= "Rp. ".number_format(($pem1->TotalPendapatan - $pem->TotalPengeluaran),2,',','.');?>
                    </th>
                   <?php }else{?>
                    Dengan ini kita mendapat Laba Kerugian Sebesar :
                        <th>
                     <?= "<b><u>Rp. ".number_format(($pem->TotalPengeluaran-$pem1->TotalPendapatan),2,',','.'); ?>
                       </th>
                 <?php }
                    ?>
                </th>
